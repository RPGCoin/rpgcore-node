...
var rpgcoin = require('rpgcoin');
var async = require('async');
var maxTime = 20;

console.log('RPGcoin Service native interface vs. RPGcoin JSON RPC interface');
console.log('----------------------------------------------------------------------');

// To run the benchmarks a fully synced RPGcoin directory is needed. The RPC comands
// can be modified to match the settings in rpg.conf.
...
var rpgd = require('../').services.RPGcoin({
  node: {
    datadir: process.env.HOME + '/.rpg',
    network: {
      name: 'testnet'
    }
  }
...
rpgd.start(function(err) {
  ...
  console.log('RPGcoin started');
});

rpgd.on('ready', function() {

  console.log('RPGcoin ready');

  var client = new rpgcoin.Client({
    host: 'localhost',
    port: 18332,
    user: 'rpg',
    pass: 'local321'
  ...
  async.series([
    function(next) {
      ...
      function rpgdGetBlockNative(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
      ...
      function rpgdGetBlockJsonRpc(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
        var hash = fixtureData.blockHashes[c];
      ...
      function rpgGetTransactionNative(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
        rpgd.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function rpgGetTransactionJsonRpc(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
      ...
      suite.add('rpgd getblock (native)', rpgdGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('rpgd getblock (json rpc)', rpgdGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('rpgd gettransaction (native)', rpgGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('rpgd gettransaction (json rpc)', rpgGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
