...
describe('RPGcoin Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, '../bin/rpgd');
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/rpg.conf'),
      rpcuser: 'rpgcoin',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/rpg.conf'),
      rpcuser: 'rpgcoin',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/rpg.conf'),
      rpcuser: 'rpgcoin',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 rpgd daemons');
    this.timeout(60000);
  ...
  it('step 1: will connect to three rpgd daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = rpgcore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.rpgd.once('tip', function(height) {
