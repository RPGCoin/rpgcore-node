...
var RPGcoinRPC = require('rpgd-rpc');
...
/**
 * Provides a friendly event driven API to rpgd in Node.js. Manages starting and
 * stopping rpgd as a child process for application support, as well as connecting
 * to multiple rpgd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function RPGcoin(options) {
  if (!(this instanceof RPGcoin)) {
    return new RPGcoin(options);
  }
...
util.inherits(RPGcoin, Service);

RPGcoin.dependencies = [];

RPGcoin.DEFAULT_MAX_TXIDS = 1000;
RPGcoin.DEFAULT_MAX_HISTORY = 50;
RPGcoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
RPGcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
RPGcoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
RPGcoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
RPGcoin.DEFAULT_SPAWN_STOP_TIME = 10000;
RPGcoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
RPGcoin.DEFAULT_REINDEX_INTERVAL = 10000;
RPGcoin.DEFAULT_START_RETRY_INTERVAL = 5000;
RPGcoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
RPGcoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
RPGcoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
RPGcoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || RPGcoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || RPGcoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || RPGcoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || RPGcoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || RPGcoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || RPGcoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || RPGcoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || RPGcoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || RPGcoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || RPGcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

RPGcoin.prototype._initCaches = function() {
...
RPGcoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
RPGcoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
RPGcoin.prototype.getPublishEvents = function() {
  return [
    {
...
RPGcoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'rpgd/' + name, 'total:', this.subscriptions[name].length);
};

RPGcoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'rpgd/' + name, 'total:', this.subscriptions[name].length);
};

RPGcoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    for(var i = 0; i < addresses.length; i++) {
        if (rpgcore.Address.isValid(addresses[i], this.node.network)) {
            addAddress(addresses[i]);
        }
    }

    log.info(emitter.remoteAddress, 'subscribe:', 'rpgd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

RPGcoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'rpgd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

RPGcoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
RPGcoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'rpgd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

RPGcoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'rpgd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
RPGcoin.prototype.unsubscribeAddressAll = function(emitter) {
...
RPGcoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = RPGcoin.DEFAULT_CONFIG_SETTINGS;
...
RPGcoin.prototype._parseRPGcoinConf = function(configPath) {
  var options = {};
...
RPGcoin.prototype._expandRelativeDatadir = function() {
...
RPGcoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in rpgd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in rpgd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in rpgd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './rpg.conf');

  log.info('Using rpgcoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseRPGcoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseRPGcoinConf(networkConfigPath));
  }
...
RPGcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of rpgcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of rpgcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of rpgcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to rpgd from rpgcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from rpgd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from rpgd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in rpg.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that rpgd is undergoing a reindex. ' +
             ...
             'of rpgcore-node services will start.');
    node._reindex = true;
  }
};

RPGcoin.prototype._resetCaches = function() {
...
RPGcoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
RPGcoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

RPGcoin.prototype._initChain = function(callback) {
  var self = this;
...
RPGcoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 7210
  };
  if (this.node.network === rpgcore.Networks.testnet) {
    networkOptions.rpcport = 17210;
  }
  return networkOptions;
};

RPGcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === rpgcore.Networks.testnet) {
    networkPath = 'testnet3/rpg.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/rpg.conf';
    }
  }
  return networkPath;
};

RPGcoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === rpgcore.Networks.testnet) {
...
RPGcoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('rpgd/hashblock', message.toString('hex'));
    }
  }

};

RPGcoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
RPGcoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('RPGcoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
RPGcoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
RPGcoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
RPGcoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('rpgd/addressbalance', {
                    address: address,
                    txid: txid,
...
RPGcoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('rpgd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
RPGcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = rpgcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('rpgd/rawtransaction', message.toString('hex'));
    }
...
RPGcoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || RPGcoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

RPGcoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
RPGcoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
RPGcoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('RPGcoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || RPGcoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

RPGcoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
RPGcoin.prototype._stopSpawnedRPGcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/rpgd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned rpgcoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean rpgcoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
RPGcoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedRPGcoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting rpgcoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('RPGcoin process unexpectedly exited with code:', code);
        log.warn('Restarting rpgcoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('RPGcoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn rpgd.'));
      }
...
RPGcoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to rpgd.'));
    }
 ...
 */
RPGcoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('RPGcoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
RPGcoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, rpgcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
RPGcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = rpgcore.Script.fromAddress(delta.address);
    return {
...
RPGcoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
RPGcoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
RPGcoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in rpgcore is most recent to less recent
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
RPGcoin.prototype._getConfirmationsDetail = function(transaction) {
...
RPGcoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
RPGcoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
RPGcoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A rpgcoin transaction id
 * @param {Function} callback
 */
RPGcoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
RPGcoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof rpgcore.Address) {
...
RPGcoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
RPGcoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
RPGcoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getRawBlock = function(blockArg, callback) {
...
RPGcoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a RPGcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
RPGcoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = rpgcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
RPGcoin.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
RPGcoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a RPGcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
RPGcoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.getInfo = function(callback) {
  var self = this;
...
RPGcoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('rpgd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('rpgd process did not exit'));
      }
...
RPGcoin.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
RPGcoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
RPGcoin.prototype.getMiningInfo = function(callback) {
    var self = this;
...
RPGcoin.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
RPGcoin.prototype.getPeerInfo = function(callback) {
    var self = this;
