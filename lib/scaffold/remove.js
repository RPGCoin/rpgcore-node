...
/**
 * Will remove a service from rpgcore-node.json
...
/**
 * Will remove the Node.js service and from the rpgcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The rpgcore-node configuration directory
...
function remove(options, done) {
  ...
  var rpgcoreConfigPath = path.resolve(configPath, 'rpgcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(rpgcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a rpgcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to rpgcore-node.json
        removeConfig(rpgcoreConfigPath, service, next);
      });
    }, done
  );
}
