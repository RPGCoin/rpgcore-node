...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for rpgd is necessary for this upgrade with the "reindex=1" rpg.conf option. \n' +
                 'There are changes necessary in both rpg.conf and rpgcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/RPGCoin/rpgcore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed rpgcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './rpgcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in rpgcore-node services directory
    ...
    if (servicePackage.rpgcoreNode) {
      serviceModule = service.name + '/' + servicePackage.rpgcoreNode;
    }
 ...
 * @param {Node} node - The RPGcore Node instance
 */
