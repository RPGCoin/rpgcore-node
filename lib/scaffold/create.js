...
var BASE_PACKAGE = {
  description: 'A full RPGcoin node build with RPGcore',
  repository: 'https://github.com/RPGCoin/rpgcore',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    "rpgcore-lib": "^1.0.3",
...
/**
 * Will create a base RPGcore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The rpgcoin database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/rpgcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a RPGcore Node directory, configuration file,
 * rpgcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the rpgcore node configuration directory
 * @param {String} options.datadir - The path to the rpgcoin datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the rpgcore-node directory and configuration
    ...
    function(next) {
      // Setup the rpgcoin directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createRPGcoinDirectory(absDataDir, next);
      } else {
        next();
      }
    },
