...
/**
 * Will return the path and default rpgcore-node configuration. It will search for the
 * configuration file in the "~/.rpgcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.rpgcore');
  var defaultConfigFile = path.resolve(defaultPath, './rpgcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['rpgd', 'web'];
